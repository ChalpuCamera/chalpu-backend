...
import com.example.chalpu.oauth.dto.LoginResponse;
import com.example.chalpu.oauth.dto.NaverLoginRequest;
import com.example.chalpu.oauth.dto.TokenDTO;
import com.example.chalpu.oauth.security.jwt.JwtTokenProvider;
import com.example.chalpu.oauth.security.oauth2.CustomOAuth2UserService;
import com.example.chalpu.oauth.security.oauth2.user.NaverOAuth2UserInfo;

import com.example.chalpu.oauth.security.oauth2.user.OAuth2UserInfo;
import com.example.chalpu.oauth.service.NaverOAuthService;
...
public class NaverAuthController {
    ...
    @Operation(
            ...
            description = """
                    ...
                    **iOS (Swift) 요청 예시:**
                    ```swift
                    guard let url = URL(string: "chalpu.com/api/auth/naver/login") else { return }
    ...
    public ResponseEntity<ApiResponse<LoginResponse>> naverMobileLogin(@RequestBody NaverLoginRequest request) {
        ...
        User user = customOAuth2UserService.processOAuth2User(oAuth2UserInfo, "naver", 
    ...
    @Operation(
            summary = "Naver 고객용 모바일 로그인/회원가입",
            description = """
                    ### Naver 모바일 SDK를 통해 고객용 로그인 후, 받은 액세스 토큰으로 서버에 로그인/회원가입을 요청하는 API 입니다.
    ...
    public ResponseEntity<ApiResponse<CustomerLoginResponse>> customerLogin(@RequestBody CustomerLoginRequest request) {
        ...
        // 3. 고객 조회 또는 생성
        Customer customer = customerService.processOAuth2Customer(oAuth2UserInfo, "naver");
        ...
        log.info("고객용 Naver 모바일 로그인 성공: customerId={}, email={}, nickname={}", 
