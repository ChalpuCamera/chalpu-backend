...
import com.example.chalpu.oauth.dto.GoogleLoginRequest;
...
import com.example.chalpu.oauth.security.oauth2.user.GoogleOAuth2UserInfo;

import com.example.chalpu.oauth.security.oauth2.user.OAuth2UserInfo;
import com.example.chalpu.oauth.service.GoogleIdentityTokenService;
...
public class GoogleAuthController {
    ...
    @Operation(
            ...
            description = """
                    ...
                    **iOS (Swift) 요청 예시:**
                    ```swift
                    guard let url = URL(string: "chalpu.com/api/auth/google/login") else { return }
    ...
    @Operation(
            summary = "고객용 Google 모바일 로그인/회원가입",
            description = """
                    ### 고객용 Google 모바일 SDK를 통해 로그인 후, 받은 ID Token으로 서버에 로그인/회원가입을 요청하는 API 입니다.
    ...
    public ResponseEntity<ApiResponse<CustomerLoginResponse>> customerLogin(@RequestBody CustomerLoginRequest request) {
        ...
        // 3. 고객 조회 또는 생성
        Customer customer = customerService.processOAuth2Customer(oAuth2UserInfo, "google");
        ...
        log.info("고객용 Google 모바일 로그인 성공: customerId={}, email={}, nickname={}", 
