package com.example.chalpu.customerfeedback.service;
...
import com.example.chalpu.customerfeedback.dto.*;
import com.example.chalpu.customerfeedback.repository.CustomerFeedbackRepository;
import com.example.chalpu.customerfeedback.repository.FeedbackPhotoRepository;
...
import com.example.chalpu.survey.repository.SurveyAnswerRepository;
import com.example.chalpu.survey.repository.SurveyRepository;
import com.example.chalpu.survey.repository.SurveyQuestionRepository;
import com.example.chalpu.common.exception.ErrorMessage;
import com.example.chalpu.common.exception.CustomerException;
import com.example.chalpu.common.exception.FeedbackException;
import com.example.chalpu.common.exception.SurveyException;
...
public class CustomerFeedbackService {
    ...
    public FeedbackResponse createFeedback(Long customerId, FeedbackCreateRequest request) {
        ...
        CustomerFeedback feedback = CustomerFeedback.createFeedback(foodItem, store, customer, survey);
    ...
    public List<FeedbackResponse> getCustomerFeedbacks(Long customerId) {
    ...
    @Transactional(readOnly = true)
    public Page<FeedbackResponse> getCustomerFeedbacks(Long customerId, Pageable pageable) {
    ...
    public FeedbackPhotosPresignedUrlResponse generateMultipleFeedbackPhotosPresignedUrl(final Long customerId, final FeedbackPhotosUploadRequest request) {
        try {
            List<FeedbackPhotosPresignedUrlResponse.FeedbackPhotoUrlInfo> photoUrls = request.getFileNames().stream()
                    .map(fileName -> {
                        try {
                            ...
                            log.error("event=single_feedback_photo_presigned_url_generation_failed, customer_id={}, file_name={}, error_message={}",
                                    customerId, fileName, e.getMessage(), e);
                            throw new FeedbackException(ErrorMessage.PRESIGNED_URL_GENERATION_FAILED);
                        }
                    })
                    .collect(Collectors.toList());
            
            log.info("event=multiple_feedback_photos_presigned_urls_generated, customer_id={}, file_count={}",
                    customerId, request.getFileNames().size());
            
            return FeedbackPhotosPresignedUrlResponse.builder()
                    .photoUrls(photoUrls)
                    .build();
        } catch (Exception e) {
            log.error("event=multiple_feedback_photos_presigned_urls_generation_failed, customer_id={}, file_count={}, error_message={}",
                    customerId, request.getFileNames() != null ? request.getFileNames().size() : 0, e.getMessage(), e);
            throw new FeedbackException(ErrorMessage.PRESIGNED_URL_GENERATION_FAILED);
        }
    }
}